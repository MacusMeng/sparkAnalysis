...
object KafkaConsumer {
  ...
  def main(args: Array[String]): Unit = {
    val ssc = new StreamingContext(SparkUtil.sparkInit("kafka Consumer")._1, Seconds(3))
      ...
      json match {
        case Some(map: Map[String, String]) =>{
          ...
          if(map.contains("meta")){
            key = map.get("meta").get
          }
          list+= WebClass(map.get("url").get,map.get("category").get,key,map.get("title").get,map.get("content").get)}
